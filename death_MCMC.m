function [RJ_MCMC_THBI_state,accepted]=death_MCMC(RJMCMC_Chain_Array_Updates,Markov_Chain)
%/************************************************************************/
% 2). Death Move-The death function deletes a layer to the current model depths
%/************************************************************************/
...
if Current_Nlayers > 1
%**************************************************************************
%  Randomly select a layer to be deleted
%**************************************************************************
    current_layer       = Current_Layers;
    Proposed_Viscous    = Current_Viscous;
	idx                 = randperm(length(current_layer),1);
%     [~,idx] = datasample(current_layer,1);
    Proposed_layers = current_layer;
    Proposed_layers(idx)=[];
    Proposed_Viscous(idx)=[];
    
    Proposed_visco = Proposed_Viscous;
...
if (isfinite(Log_Likelihood_Current) && isfinite(Log_Likehood_Proposed))    
    if (Log_Alpha_Ratio >0 || Log_Alpha_Ratio > Death)
        accepted = 1;
        ...
        RJ_MCMC_THBI_state.ADeath_count         = 1;
        RJ_MCMC_THBI_state.RDeath_count         = 0;
        ...
        accepted = 0;
        RJ_MCMC_THBI_state_old.ADeath_count     = 0;
        RJ_MCMC_THBI_state_old.RDeath_count     = 1;
